...
describe("Asyncs action to add user", () => {
	beforeEach(() => {
		...
		localStorage.setItem("access_token", JSON.stringify("eyJ0eXAiOiJKV1QiLSa78Q9ZF5fjbfIsfhS1d-6wCyQ"));
	});
	afterEach(() => {
		fetchMock.reset();
		fetchMock.restore();
	...
	it("Register user", () => {
		const data = {email:"ezrqnkemboi@gmail.com", first_name:"ezrqn",
			last_name:"kemboi", username:"ezrqn", password:"ezrqnpassword"};
	...
	it("It logs in a user", () => {
		...
		const expectedAction = [
			...
			{type:userConstants.LOGIN_SUCCESS, body:{user:user}}
		];
		const store = mockStore({user:user}, expectedAction);
		fetchMock.mock("*", {user:user});
	...
	it("It reset user password", () => {
		...
		const expectedAction = [
			...
			{type:userConstants.RESET_PASSWORD_SUCCESS, body:{user:user}}
		];
		const store = mockStore({user:user}, expectedAction);
		fetchMock.mock("*", {user:user});
